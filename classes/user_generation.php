...
if ( !empty($_POST['showUsers']) )
	$_SESSION['bridge']['users_page_limit'] = $_POST['showUsers'];

if ( !empty($_POST['showLevels']) )
	$_SESSION['bridge']['levels_page_limit'] = $_POST['showLevels'];
...
function displayUsers($row) {
	...
	?>
	<tr>
		<td><a href="nprofile.php?id=<?php echo $row['user_id']; ?>"><?php echo $generic->get_gravatar($email, true, 20, 'mm', 'g', array('style' => '1')); ?> <?php echo $row['username']; ?></a></td>
...
function pagination($table, $args = '',$total_pages = '') {
	...
	if($table == 'login_levels') :
		$hash = '#level-control';
		if (!empty($_SESSION['bridge']['levels_page_limit']))
			$limit = $_SESSION['bridge']['levels_page_limit'];
	endif;

	if($table == 'login_users') :
		$hash = '#user-control';
		if (!empty($_SESSION['bridge']['users_page_limit']))
			$limit = $_SESSION['bridge']['users_page_limit'];
	endif;
