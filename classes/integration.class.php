...
class bridge_integration extends Generic {
	...
	function __construct() {
		...
		if (empty($_SESSION['bridge']['user_id'])) {
			//$this->guestLogin();
			return false;
		}
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['bridge']['user_id'] );
	...
	private function connect($link) {
		...
		$params = array(
			':user_id'      => $_SESSION['bridge']['user_id'],
			':session_link' => $_SESSION['bridge'][$link]
		);
	...
	private function unlink($provider) {
		...
		unset($_SESSION['bridge'][$provider]);
			...
			if(!$openid->mode) {
				...
				if ($openid->validate()) :
					...
					if( !empty($providers[$link]) ) :
						...
						$_SESSION['bridge'][$link] = $openid->identity;
						$this->connect($link);
						$_SESSION['bridge']['openIDMisc'] = $openid->getAttributes();
						$_SESSION['bridge']['openIDMisc']['type'] = $link;
						$_SESSION['bridge']['openIDMisc'][$link] = $openid->identity;
	...
	public function facebook_url() {
		...
		if( !empty($_SESSION['bridge']['facebook']) )
			return false;
		...
		if ($this->fbUser) :
		  ...
		  try {
			$_SESSION['bridge']['facebookMisc'] = $user_profile;
			$_SESSION['bridge']['facebook'] = $this->fbUser;
	...
	public function twitter_url() {
		...
		if ( !empty($_SESSION['bridge']['twitter']) )
			return false;
		...
		if( !empty($_GET['oauth_token']) ) {
			...
			$_SESSION['bridge']['twitter'] = $twitterInfo['id'];
			$_SESSION['bridge']['twitterMisc']['username'] = $twitterInfo['screen_name'];
			$_SESSION['bridge']['twitterMisc']['id'] = $twitterInfo['id'];
			$_SESSION['bridge']['twitterMisc']['name'] = $twitterInfo['name'];

		}

		return !empty($_SESSION['bridge']['twitter']) ? '#' : $this->twitterObj->getAuthenticateUrl( null,array('oauth_callback' => str_replace("www.","", SITE_PATH) . basename($_SERVER['PHP_SELF']) . '?link=twitter' ));
...
$bridge_integration = new bridge_integration();