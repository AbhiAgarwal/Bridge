...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['bridge']['user_id'] );
		...
		if ( empty($_SESSION['bridge']['user_id']) ) :
			...
			$_SESSION['bridge']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['bridge']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['bridge']['restricted']) || !empty($_SESSION['bridge']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['bridge']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['bridge']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['bridge']['user_id'] ) && $level == array('*') )
			return false;
