...
class Generic extends Connect {
	...
	public function profileFieldTypes($option, $signup = false, $user_id = '') {
		...
		<div class="control-group">
			<h1 style="color:#800080; font-family:Gotham; font-size:45px; position:left;"><?php echo $name; ?></h1></label>
			<div class="controls">
		<?php
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['bridge']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function guestOnly() {
		...
		if(!empty($_SESSION['bridge']['username'])) {
			$this->error =	"
	...
	public function generateToken() {
		...
		if(empty($_SESSION['bridge']['token']))
			$_SESSION['bridge']['token'] = md5(uniqid(mt_rand(),true));
	...
	public function valid_token($token) {
		...
		if ($_SESSION['bridge']['token'] != $token)
			return false;
	...
	public function definePaths() {
		...
		if (!defined('phplogin_version'))       define( 'phplogin_version',    3.11);
