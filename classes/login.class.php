...
class Login extends bridge_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['bridge']['referer'] = (!empty($_SESSION['bridge']['referer'])) ? $_SESSION['bridge']['referer'] : 'home.php';
		else
			$_SESSION['bridge']['referer'] = parent::getOption('signin-redirect-url');
		...
		foreach (parent::$socialLogin as $provider) :
			if (!empty($_SESSION['bridge'][$provider])) {
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['bridge'][$provider] );
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['bridge']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['bridge']['restricted'] = 1;
		...
		if (parent::getOption('pw-encrypt-force-enable')) :
			...
			if (strlen($this->result['password']) == 32 && $type == 'SHA256')
				$_SESSION['bridge']['forcePwUpdate'] = 1;
		...
		$_SESSION['bridge']['user_level'] = $user_level;

		$_SESSION['bridge']['email'] = $this->result['email'];

		$_SESSION['bridge']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['bridge']['level_disabled'] = 1;
		...
		$_SESSION['bridge']['username'] = $this->result['username'];

		// User ID of the logging in user
		$_SESSION['bridge']['user_id'] = $this->result['user_id'];

		if ( empty($redirect) ) $redirect = $_SESSION['bridge']['referer'];

		unset(
			$_SESSION['bridge']['referer'],
			$_SESSION['bridge']['token'],
			$_SESSION['bridge']['facebookMisc'],
			$_SESSION['bridge']['twitterMisc'],
			$_SESSION['bridge']['openIDMisc']
		);
