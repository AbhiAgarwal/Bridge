...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		if ( !empty($_SESSION['bridge']['openIDMisc']) ) :

			$misc = $_SESSION['bridge']['openIDMisc'];
		...
		if ( !empty($_SESSION['bridge']['facebookMisc']) ) :

			$misc = $_SESSION['bridge']['facebookMisc'];
		...
		if ( !empty($_SESSION['bridge']['twitterMisc']) ) :

			$misc = $_SESSION['bridge']['twitterMisc'];
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$_SESSION['bridge']['username'] = $this->settings['username'];

			/* Apply default user_level */
			$_SESSION['bridge']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['bridge']['activate'] = 1;

			$_SESSION['bridge']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
			...
			$_SESSION['bridge']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['bridge']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['bridge']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['bridge']['openIDMisc']) ) {
				$link = $_SESSION['bridge']['openIDMisc']['type'];
				$id = $_SESSION['bridge']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['bridge']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['bridge']['twitterMisc']['id'];
			}
